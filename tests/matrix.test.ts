...
describe('utils', () => {
    ...
    test('mulVector', () => {
        ...
        // Test multiplying by identity up to N=10, M=10x10
        const arr = new Array(10).fill(0).map((_, i) => i);
        for (let index = 1; index <= arr.length; index++) {
            const sub = arr.slice(0, index);
            expect(new Vector(sub).transform(Matrix.identity(index)).values).toEqual(sub);
            expect(Matrix.identity(index).mulVector(new Vector(sub)).values).toEqual(sub);
        }
